...
import { AlertTriangle, CheckCircle, ChevronLeft, Send } from "lucide-react";
import { pushToTally } from "../utils/api.js";
...
export default function ReviewScreen({ data: initialData, onPushed, onBack }) {
  ...
  async function handlePush() {
    ...
    setError("");
    try {
      const result = await pushToTally(data);
      onPushed(result);
    } catch (err) {
      ...
      <div className="fade-up" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
        <div>
          ...
          <p style={{ color: "var(--text-muted)", fontSize: 13, marginTop: 3 }}>
            PO {data.meta?.sfPONumber} · {data.meta?.emailPOFormat} format ·{" "}
            <span style={{ color: data.meta?.docsLinked ? "var(--success)" : "var(--warn)" }}>
              {data.meta?.docsLinked ? "Documents linked ✓" : "⚠ Documents not linked"}
            </span>
          </p>
        </div>
        <button
