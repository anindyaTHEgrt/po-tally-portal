...
export default function UploadScreen({ onParsed }) {
  ...
  const canParse = sfFile && emailFile && !loading;
  ...
  async function handleParse() {
    ...
    setError("");
    try {
      const result = await uploadPDFs(sfFile, emailFile);
      if (result.success) {
      ...
      setError(err.response?.data?.error || err.message || "Failed to parse PDFs.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div style={{
      ...
      <div className="fade-up" style={{ marginBottom: 40, textAlign: "center" }}>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: 14, lineHeight: 1.7 }}>
          Drop both PDFs — the Salesforce PO and the vendor email PO.<br />
          We'll parse and match them automatically.
        </p>
      </div>
      ...
      <div className="fade-up fade-up-1" style={{
        ...
        />
        <DropZone
          label="Vendor Email PO"
          hint="Borkar or Parksons format — auto-detected"
          file={emailFile}
          onFile={setEmailFile}
        />
      </div>
      ...
      <div className="fade-up fade-up-2" style={{
        ...
        Email PO format is auto-detected — Borkar Packaging or Parksons Packaging.
        Parksons parser is a stub until a sample PO is provided.
      </div>
